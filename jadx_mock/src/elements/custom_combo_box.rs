...
use orbtk::prelude::*;
use orbtk::prelude::{
    behaviors::{MouseBehavior, SelectionBehavior},
    themes::{colors, material_icons_font, orbtk_fonts},
};
...
impl State for ComboBoxItemState {
    fn update(&mut self, _: &mut Registry, ctx: &mut Context) {
        ...
        ctx.get_widget(self.combo_box).set("selected_item", Some(entity));
        ...
        if let Some(builder) = &self.builder {
            ...
            build_context.register_shared_property::<Brush>("foreground", selected_content, self.combo_box);
            build_context.register_shared_property::<f32>("opacity", selected_content, self.combo_box);
            build_context.register_shared_property::<f64>("font_size", selected_content, self.combo_box);
...
widget!(
    ...
    CustomComboBoxItem<ComboBoxItemState>: MouseHandler {
...
impl Template for CustomComboBoxItem {
    fn template(self, id: Entity, ctx: &mut BuildContext) -> Self {
        self.name("CustomComboBoxItem")
            .style("custom_combo_box_item")
            .min_width(64)
            .height(24)
            .selected(false)
            .pressed(false)
...
#[derive(Default, AsAny)]
pub struct CustomComboBoxState {
    popup: Entity,
...
impl State for CustomComboBoxState {
    fn update(&mut self, _: &mut Registry, ctx: &mut Context) {
            ...
            if let Some(builder) = &self.builder {
                ...
                for i in 0..count {
                    let item = {
                        ...
                        let mut item = CustomComboBoxItem::new()
                            .index(i)
                            .combo_box(entity)
...
widget!(
    ...
    ///
    /// **style:** `combo_box`
    CustomComboBox<CustomComboBoxState>: MouseHandler {
        /// Sets or shares the background property.
        background: Brush,
...
impl Template for CustomComboBox {
    fn template(mut self, id: Entity, ctx: &mut BuildContext) -> Self {
        ...
        let height: usize;
        let count_prop = self.count.as_ref().expect("msg");
        match count_prop {
            PropertySource::Source(_) => height = 0,
            PropertySource::KeySource(_, _) => height = 0,
            PropertySource::Value(t) => height = *t,
        }

        let popup = Popup::new()
            .height(height as f64 * 24.0)
            .open(("selected", id))
            .child(scroll_viewer)
            .child(
                ScrollIndicator::new()
        ...
        self.name("CustomComboBox")
            ...
            .on_global_mouse_up(move |states, e| {
                states
                    .get_mut::<CustomComboBoxState>(id)
                    .action(Action::CheckMouseUpOutside { position: e.position })
            })
    }
}
