...
use orbtk::{
    prelude::{
        ...
        themes::{colors, material_icons_font},
    },
    shell::{Key, KeyEvent},
};
...
pub struct SmallNumericBoxState {
...
impl State for SmallNumericBoxState {
    fn init(&mut self, _: &mut Registry, ctx: &mut Context) {
        self.input = ctx.entity_of_child(ID_INPUT).expect(
            "SmallNumericBoxState
...
widget!(
    ...
    /// ```
    SmallNumericBox<SmallNumericBoxState>: ActivateHandler, KeyDownHandler {
...
impl Template for SmallNumericBox {
    fn template(self, id: Entity, ctx: &mut BuildContext) -> Self {
        self.name("SmallNumericBox")
            .style("numeric_box")
            ...
            .child(
                MouseBehavior::new()
                    .on_mouse_down(move |states, _| {
                        states
                            .get_mut::<SmallNumericBoxState>(id)
